...
class ActorEvents_16 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				Engine.engine.setGameAttribute("X of Ship", actor.getX());
				Engine.engine.setGameAttribute("Y of Ship", actor.getY());
			}
		});
		...
		addKeyStateListener("action1", function(pressed:Bool, released:Bool, list:Array<Dynamic>):Void
		{
			...
			{
				createRecycledActor(getActorType(14), actor.getX(), actor.getY(), Script.FRONT);
